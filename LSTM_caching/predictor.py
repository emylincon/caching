...
cpu_1 = psutil.cpu_percent(interval=0.5)
mem_1 = system.memory_percent()
...
def resource_record():
    max_length = 200
    cpu_2 = psutil.cpu_percent(interval=0.5)
    mem_2 = system.memory_percent()
    avg_cpu = abs(cpu_2 - cpu_1)
    avg_mem = abs(mem_2 - mem_1)

    cpu_dir = f'results/cpus'
    mem_dir = f'results/mems'
    cpu_list = os.listdir(cpu_dir)
    cpu_list.remove('__init__.py')

    if len(cpu_list) == 0:
        save_file(filename=f'{cpu_dir}/00.pickle', data=[avg_cpu])
        save_file(filename=f'{mem_dir}/00.pickle', data=[avg_mem])
        ...
        if len(loaded_cpu) >= max_length:
            ...
            create_file(f'{mem_dir}/{fname}')
        save_file(f'{cpu_dir}/{cfile}', loaded_cpu)
        save_file(f'{mem_dir}/{cfile}', loaded_mem)
