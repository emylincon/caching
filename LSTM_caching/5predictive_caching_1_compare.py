...
class LocalCache:
    ...
    @staticmethod
    def get_popularity(request, t_time):
        cmd = [f'python3 predictor.py --r={request} --t="{t_time}"']
    ...
    def push(self, request, t_time):
        if request in self.cache:
            ...
            popularity = self.get_popularity(request, t_time)
...
def send_email(msg, size):
    try:
        server = smtplib.SMTP_SSL('smtp.gmail.com')
        server.ehlo()
        server.login(config.email_address, config.password)
        subject = 'Caching results {} {} size={}'.format('LSTM caching', get_hostname(), size)
...
def send_email_attachment(file, size):
    msg = EmailMessage()

    msg['Subject'] = 'Caching results {} {}, size={}'.format('LSTM caching', get_hostname(), size)
...
def save_data(mem, cpu, delay, hit_ratio, no, cache_size):
    host = get_hostname()
    host_no = int(re.findall('[0-9]+', host)[0])
    data = f"""
    memory{host_no}_{no}_{cache_size} = {mem}
    cpu{host_no}_{no}_{cache_size} = {cpu}
    delay{host_no}_{no}_{cache_size} = {delay}
    hit_ratio{host_no}_{no}_{cache_size} = {hit_ratio}
    """
    send_email(data, cache_size)
    file = open(f'results/output{host_no}_{no}_{cache_size}.py', 'w')
    file.write(data)
    file.close()
    send_path = '/home/student/results/LSTM/'
    sp.run(
        ["scp", f'results/output{host_no}_{no}_{cache_size}.py', f"student@192.168.122.84:{send_path}"])
    for res in ['memory', 'cpu', 'delay', 'mems', 'cpus']:
        os.system(f'zip results/{res}{host_no}_{no}_{cache_size}.zip results/{res}/*')
        sp.run(
            ["scp", f'results/{res}{host_no}_{no}_{cache_size}.zip', f"student@192.168.122.84:{send_path}"])
...
def initialization():
    br = BrokerRequest(user='mec', pw='password', ip='192.168.122.84', sub_topic='control')
    br.broker_loop()
    del br
    print('starting ....')
...
def run(no_mec):
    ...
    no_of_requests = 3000       # No of requests should be divisible by 5, 10, 15 MECs |  67,200
    ...
    def experiment(cache_size):
        ...
        store = LocalCache(cache_size=cache_size, delay=network_cost_record)
