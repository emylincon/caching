...
def process_request(request, t_time):
    if request in trained:
        model = get_model(f'models/model{request}.sav')
        raw_data = pd.read_csv(f'data/{request}.csv')
        stop = np.where(raw_data['timestamp'] == t_time)[0][-1]
        if stop >= 80:
            data = list(raw_data['rating'].values[stop-80:stop])
            data = [[i] for i in data]
            print('predicted->', predict_next(model=model, data=data))
        else:
...
def run():
    ...
    for i in range(request_data.shape[0]-no_of_requests, request_data.shape[0]):
        process_request(request_data['movieId'][i], request_data['timestamp'][i])
