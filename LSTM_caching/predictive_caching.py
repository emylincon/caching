...
class LocalCache:
        ...
        if request in self.cache:
            ...
            if len(self.cache) >= self.cache_size:
                ...
                if self.cache[min_pop_cache] <= popularity:
                    ...
                    # fetch data
                    # data = self.get_data(f'http://{request}.html')
                    data = f'hello {request}'
                    self.cache_data(request=request, data=data)
                    self.cache[request] = popularity
                else:
    ...
    def cache_data(self, request, data):
        file = open(f'{self.cache_dir}/{request}', 'w')
        file.write(data)
        file.close()
